...
def messagerie(request, projet_id=None):
    ...
    if request.method == "POST":
        form = MessageForm(
            request.POST, request.FILES, user=request.user, projet=projet
        )
        if form.is_valid():
            objet = form.cleaned_data["objet"]
            messages = form.cleaned_data["messages"]
            ...
            msg.save()
            if message_predefini:
                role_user = projet.get_user_by_type_choice(
                    message_predefini.destinataire_role
                )
...
def courier_entrant(request, projet_id=None):
    ...
    if projet_id:
        ...
        messages = Message.objects.filter(
            recepteurs=request.user, projet=projet
        ).order_by("-date_envoie")
    else:
        messages = Message.objects.filter(recepteurs=request.user).order_by(
            "-date_envoie"
        )
...
def courier_envoye(request, projet_id=None):
    ...
    if projet_id:
        ...
        messages_envoyes = Message.objects.filter(
            emetteur=request.user, projet=projet
        ).order_by("-date_envoie")
        context["projet_id"] = projet_id
        context["projet"] = projet
    else:
        # Sinon, montrer tous les messages envoyés par l'utilisateur
        messages_envoyes = Message.objects.filter(emetteur=request.user).order_by(
            "-date_envoie"
        )
...
def reply_to_message(request, message_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            send_notification_email(
                new_message.recepteurs.all(), new_message.objet, new_message.messages
            )
    ...
    return render(
        request, "reply.html", {"form": form, "original_message": original_message}
    )
