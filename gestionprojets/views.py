...
def ganttchartprojects(request):
    ...
    for projet in projets:
        ...
        tooltip = (
            f"Coordinateur: {projet.coordinateur.get_full_name()}, "
...
def Taskliste(request):
    ...
    ongoing_tasks = user_tasks.filter(
        start_date__lte=date.today(), end_date__gte=date.today(), status=2
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            ActionHistory.objects.create(
                ...
                entity_type="Client",
                entity_id=client_id,
                action_details={"old_data": {}, "new_data": new_data},
            )
...
def listeClient(request):
    ...
    context = {"clients": clients, "search_term": search_term}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            ActionHistory.objects.create(
                ...
                entity_type="Projet",
                entity_id=projet.id,
                action_details={"old_data": old_data, "new_data": new_data},
            )
...
def deletteProjet(request, pk):
    ...
    ActionHistory.objects.create(
        ...
        entity_type="Projet",
        entity_id=projet_id,
        action_details={"old_data": old_data, "new_data": {}},
    )
    return redirect("projectmanagement:projectlist")
...
def listeProject(request):
    ...
    if query:
        projets_list = projets_list.filter(name__icontains=query)
    page = request.GET.get("page", 1)
    paginator = Paginator(projets_list, 10)
    try:
...
def manage_agents(request, pk):
    ...
    users_info = [{"user": user, "role": projet.get_user_role(user)} for user in users]
    if request.method == "POST":
...
def List_Intervenant_Project(request, pk):
    ...
    users_info = [
        {"user": user, "role": projet.get_user_role(user)} for user in intervenants
    ]
    context = {
        "projet": projet,
        "status": projet.get_status_display(),
...
def newTask(request, pk):
    ...
    if request.method == "POST":
        form = (
            ...
            if request.user.is_chefDeProjet_or_coordinateur_or_admin()
            else TaskLimitedForm(request.POST, request.FILES)
        )
        if form.is_valid():
            ...
            try:
                ...
                form = (
                    TaskForm(projet=projet)
                    if request.user.is_chefDeProjet_or_coordinateur_or_admin()
                    else TaskLimitedForm()
                )
    else:
        form = (
            TaskForm(projet=projet)
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect(
                    reverse("projectmanagement:detailprojet", args=(task.projet.pk,))
                )
        return HttpResponseRedirect(
            reverse("projectmanagement:detailprojet", args=(task.projet.pk,))
        )
...
def new_phase_for_project(request, project_id):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "projectmanagement:list_phases_for_project", project_id=project_id
            )
    else:
        form = PhaseForm()
...
def edit_phase(request, phase_id, projet_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "projectmanagement:list_phases_for_project", project_id=projet_id
            )
    else:
